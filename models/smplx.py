...
def batch_global_rigid_transformation(Rs, Js, parent, rotate_base=False):
    ...
    new_J = results[:, :, :3, 3]
    Js_w0 = torch.cat([Js, torch.zeros(N, num_joins, 1, 1).to(device)], dim=2)
...
class SMPLX(nn.Module):
    def __init__(self, model_path, is_smplx=True, use_pca=True, num_pca_comps=45, hand_mean=True, use_face_contour=True, betas_dim=300, expr_dim=100):
        super(SMPLX, self).__init__()
        if model_path.endswith('pkl'):
            with open(model_path, 'rb') as f:
                smpl_model = pickle.load(f, encoding='latin1')
        elif model_path.endswith('npz'):
            smpl_model = np.load(model_path)
        ...
        if is_smplx:
            num_beta = betas_dim
            exprdirs = shapedirs[...,300:300+expr_dim]
            # exprdirs = np.reshape(exprdirs, [-1,expr_dim]).T
            self.register_buffer('exprdirs', torch.FloatTensor(exprdirs))
    ...
    def forward(self, pose, shape, v_template=None, trans=None, scale=1.0, delta=None):
        b = pose.shape[0]
        device = pose.device
        ...
        v_shaped = torch.einsum('bl,mkl->bmk', shape, shapedirs) + v_template
        ...
        T = torch.matmul(weights, A.reshape(b, -1, 16)).reshape(b, -1, 4, 4)
    ...
    def get_skinning(self, pose, shape, v_template=None):
        b = pose.shape[0]
        if v_template is None:
            v_template = self.v_template
        if self.is_smplx:
